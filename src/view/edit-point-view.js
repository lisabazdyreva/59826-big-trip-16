...
import {FAKE_NAMES, TYPES} from '../consts';
...
const getEventTypeListTemplate = (activeType) => `<div class="event__type-list">
  <fieldset class="event__type-group">
    ...
    ${TYPES.map((type) => `<div class="event__type-item">
      <input id="event-type-${type}-1" class="event__type-input  visually-hidden" type="radio" name="event-type" value=${type} ${activeType === type ? 'checked': ''}>
      <label class="event__type-label  event__type-label--${type}" for="event-type-${type}-1">${type.slice(0, 1).toUpperCase() + type.slice(1)}</label>
    </div>`).join('')}
  </fieldset>
</div>`;
...
const getTimeTemplate = (dateFrom, dateTo) => {
  const from = dayjs(dateFrom).format('DD/MM/YY HH:mm');
  const to = dayjs(dateTo).format('DD/MM/YY HH:mm');
...
const getDestinationTemplate = (description, picturesTemplate) => `<section class="event__section  event__section--destination">
  <h3 class="event__section-title  event__section-title--destination">Destination</h3>
  ${description ? `<p class="event__destination-description">${description}</p>` : ''}
  ${isEditPoint ? picturesTemplate : ''}
</section>`;
...
const createEditPointView = (point = {}) => {
  ...
  const destinationTemplate = description || pictures.length ? getDestinationTemplate(description, picturesTemplate) : ''; //TODO не передаю тип формы
